...
import {
  IJwtDecoded,
  IJwtPayload,
} from '@shared/interface/jwt-payload.interface';
...
export class JwtStrategy extends PassportStrategy(
  ...
  constructor(
    ...
    super({
      ...
      secretOrKeyProvider: (request: Request, rawJwtToken: string, done) => {
        const decoded = this.jwtService.decode<IJwtDecoded>(rawJwtToken, {
          complete: true,
        });

        if (!decoded || typeof decoded !== 'object')
          return done(
            new Error(getErrorMessage(ErrorCode.InvalidDecodeToken)),
            undefined,
          );

        const { iss } = decoded.payload;

        if (iss === TokenIssuer.Access)
          return done(null, ENVIRONMENT.JWT_SECRET);
        if (iss === TokenIssuer.Sensitive)
          return done(null, ENVIRONMENT.JWT_SENSITIVE_SECRET);

        return done(
          new Error(getErrorMessage(ErrorCode.InvalidTokenIssuer)),
          undefined,
        );
      },
    });
  ...
  async validate(payload: IJwtPayload) {
    const cachedUser = await this.redisService.get<UserModel>(
      `${RedisKey.Account}${payload.sub}`,
    );
    if (cachedUser) {
