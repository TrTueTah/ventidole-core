...
export class VerificationConsumer extends WorkerHost {
  ...
  async process(job: Job<unknown, unknown, string>) {
    try {
      ...
      WinstonLogger.error(
        getErrorMessage(ErrorCode.ConsumerFailed, job.id, job.name),
        { metadata: error },
      );
      throw error;
    }
  ...
  onActive(job: Job) {
    WinstonLogger.info(
      getMessage(MessageCode.JobProcessing, job.id, job.name),
      { metadata: { data: job.data } },
    );
  }
  ...
  onFailed(job: Job, error: Error) {
    WinstonLogger.error(
      getErrorMessage(ErrorCode.ProcessFailed, job.id, job.name, error.message),
      {
