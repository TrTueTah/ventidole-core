...
const Message: Record<MessageCode, string | ((...param: unknown[]) => string)> =
  {
    [MessageCode.ServerStarted]: (port: number) =>
      `✅ Server ready at http://localhost:${port}`,
    [MessageCode.ServerStartFailed]: (error) =>
      `❌ Error while starting server, ${error}`,

    [MessageCode.RedisConnected]: `${redisColorized} Connected`,
    [MessageCode.RedisFailedToConnect]: (msg: string) =>
      `${redisColorized} Failed to connect: ${msg}`,
    ...
    [MessageCode.JobProcessing]: (jobId: string, jobName: string) =>
      `Processing job [${jobName}] id: ${jobId}`,
    [MessageCode.JobCompleted]: (jobId: string, jobName: string) =>
      `Complete job [${jobName}] id: ${jobId}`,
    ...
    [MessageCode.CronJobStarted]: (jobName: string, startedAt: string) =>
      `Job [${jobName}] started at ${startedAt}`,
    [MessageCode.CronJobCompleted]: (jobName: string, completedAt: string) =>
      `Job [${jobName}] completed at ${completedAt}`,
    [MessageCode.CronJobFailed]: (jobName: string, failedAt: string) =>
      `Job [${jobName}] failed at ${failedAt}`,
  } as const;
